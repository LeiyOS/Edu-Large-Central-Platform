...
| 令牌（CSS 变量）                | 色值        | 用途                          |
| ------------------------- | --------- | --------------------------- |
| `--color-primary`         | `#F10C0C` | 主按钮、分页当前页、通知角标、筛选聚焦环、链接强调色等 |
...
| 表格分割线（列表页）       | `rgba(15, 23, 42, 0.055)` 等（`--hairline`） | 表头 / 单元格底边                      |
...
| 列表主按钮    | `btn-primary`                 | 15px                                    | 600          | 白字 on 主色                   |
...
| Logo     | `position: fixed`，高度约 `40px`，含 `safe-area`                                       |
...
| 账号列表外层卡片、筛选行                         | **12px** | `filter-field__line`、`account-panel__table-box`             |
...
- 白滑块 + 轻阴影；位移动画 `transform` + `cubic-bezier(0.25, 0.8, 0.25, 1)`。
...
- Vant `van-button`，不用 `round`；Hover `brightness`；Active `scale(0.99)`。

### 7.4 列表页：筛选行（`filter-field__line`）

- 白底、**12px** 圆角、浅描边、轻阴影。
- `focus-within`：**主色描边** + **外扩光晕**（`box-shadow` 多层）。
- 文本用 **底边无** 的 `input`；下拉 `**appearance: none`** + 右侧 **SVG 下箭头** 背景图。

### 7.5 列表页：按钮

- **主按钮** `btn-primary`：主色实底，**12px** 圆角，高 **44px**。
- **幽灵主按钮** `btn-primary--ghost`：浅主色透明底 + 主色字（如「添加新账号」）。
- **线框按钮** `btn-outline`：浅灰描边，白底。
...
### 7.7 分页（`van-pagination`）

- `mode="multi"`；**每项独立圆角块**（约 **10px**），默认描边，**当前页主色实底白字**；`:deep` 去掉 Vant 默认连体描边（`::after`）。

### 7.8 管理后台壳层

- **顶栏**：业务 Tab 带 **chevron-down**（示意下拉）；右侧通知 **主色角标**。
- **侧栏**：分组间 **1px 分隔线**（伪元素）；当前项 **半透明白底 + 轻阴影 + 内高光描边**。
- **账号块**：白卡片；**chevron** 使用 `lucide:chevron-down`，样式上 `**transform: rotate(-90deg)`** 呈现为指向右侧（与实现一致）。
- **退出登录**：浅灰底 **10px** 圆角条。

### 7.9 无障碍

- 表单控件 `label` / `id` 关联；侧栏 FAB 带 `aria-expanded`、`aria-controls`；可聚焦控件 `**focus-visible` 使用 `--color-focus-ring`**。
...
- 主按钮与筛选 / Tab 是否为 **12px～14px** 圆角矩形，而非默认胶囊
...
| `src/components/LoginPage.vue`             | 登录布局、分段器、底边线输入      |
| `src/components/admin/AdminLayout.vue`     | 侧栏 / 顶栏 / 主区槽位、留白变量 |
| `src/components/admin/AccountListPage.vue` | 筛选、表格、状态、分页、列表按钮    |
